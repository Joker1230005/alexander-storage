...
function New-Configuration($keys) {
    ...
    $dataPathForward = $DATA_DIR -replace '\\', '/'
...
database:
  ...
  path: "$dataPathForward/data/alexander.db"
  journal_mode: "WAL"
  busy_timeout: 5000
  cache_size: -2000
...
storage:
  backend: "filesystem"
  filesystem:
    base_path: "$dataPathForward/blobs"
...
function Install-WindowsService {
    ...
    if ($nssmPath) {
        ...
        $wrapperLines | Out-File -FilePath "$INSTALL_DIR\alexander-service.bat" -Encoding ASCII
...
function New-AdminUser {
    ...
    try {
        ...
        Write-Host "===========================================================" -ForegroundColor Green
        Write-Host ""
        Write-Host "  WARNING: Save this password! It will not be shown again." -ForegroundColor Red
        Write-Host ""
    }
    catch {
...
function New-AccessKey {
    ...
    try {
        ...
        if ($output) {
            ...
            Write-Host "===========================================================" -ForegroundColor Green
            Write-Host ""
            Write-Host "  WARNING: Save these keys! They will not be shown again." -ForegroundColor Red
            Write-Host ""
        }
    }
    catch {
